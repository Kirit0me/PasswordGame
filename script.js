...
function elementSum(passwd){
    let sum=0;
    for (symbol in periodicTable){
        if (passwd.includes(symbol)){
            atomicNumber = periodicTable[symbol];
            let count = passwd.match(symbol).length;
            sum=sum+(atomicNumber*count);
        }
        else{
            continue;
        }
